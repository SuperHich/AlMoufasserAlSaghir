...
public class CustomizedCalendarCells extends ImageButton {
	...
	public CustomizedCalendarCells(Context context) {
		super(context);
		init();
	}

	public CustomizedCalendarCells(Context context, AttributeSet attrs) {
		super(context, attrs);
		parseAttrs(attrs);
		init();
	}

	public CustomizedCalendarCells(Context context, AttributeSet attrs, int defStyle) {
...
//		mTextPaint.setTextScaleX(1.0f);
		...
		float target = (float) mViewHeight * .4f;
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (CalendarActivity.day_color[1].equals("GREY")) {
			mTextPaint.setColor((Color.parseColor("#747a81"))); //// jours precedents  #747a81
		}
		if (CalendarActivity.day_color[1].equals("WHITE")) {
			mTextPaint.setColor((Color.parseColor("#121e2a"))); ///jours normals  #121e2a
		}
		if (CalendarActivity.day_color[1].equals("BLUE")) {
			mTextPaint.setColor(getResources().getColor(R.color.lightgray));
		}
...
//		adjustTextScale();
	...
	protected void onDraw(Canvas canvas) {
		...
		// view bottom
		canvas.drawText(mText, mViewWidth / 2, mViewHeight - mTextBaseline-8,
				mTextPaint);
